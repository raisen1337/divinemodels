...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    return {
        title: `Delete Image - ${model?.name || 'Model'}`,
        description: `Delete image from ${model?.name || 'model'}'s portfolio`,
    };
}

export default async function DeleteImagePage({ params }: Props) {
    ...
    return (
        <div className="p-6">
            <div className="flex items-center justify-between mb-8">
                <div>
                    <h1 className="text-3xl font-semibold">Delete Image</h1>
                    <p className="text-gray-600 mt-2">Remove image from {model.name}'s portfolio.</p>
                </div>
            </div>
